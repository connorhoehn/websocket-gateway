...
- [ ] **Phase 6: Foundation** - React+Vite scaffold, useWebSocket hook, auth config, connection status UI
- [ ] **Phase 7: Presence & Cursors** - usePresence + PresencePanel, useCursors + CursorCanvas
...
**Depends on**: Phase 6
**Requirements**: PRES-01, PRES-02, PRES-03, CURS-01, CURS-02, CURS-03
**Success Criteria** (what must be TRUE):
  ...
  3. Moving the mouse on one tab causes a labeled cursor with a distinct per-user color to appear on the shared canvas in all other tabs
  4. When a tab disconnects, its cursor fades out and disappears from the canvas in the remaining tabs
**Plans**: TBD

Plans:
- [ ] 07-01: usePresence hook + PresencePanel component (user list + typing indicators)
- [ ] 07-02: useCursors hook + CursorCanvas component (broadcast, render, fade-out)
...
| 6. Foundation | v1.2 | 0/3 | Not started | - |
